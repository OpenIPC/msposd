...
const int RC_CHANNELS_RAW = 35; // RC_CHANNELS_RAW ( #35 ) for ExpressLRS,Crossfire and other RC
								// procotols
...
/// @brief wfb_tx output should be redirected to wfb.log. Parse it and extracted
/// dropped packets!
...
int SendWfbLogToGround() {
	...
	*/
	...
	while (fgets(buff, sizeof(buff), file) != NULL) {
		...
		if (strstr(buff, "packets dropped") !=
			NULL) {							 // Check if the line contains "packets dropped"
			char *token = strtok(buff, " "); // split by space,  Parse the line to
											 // extract the number of dropped packets
			while (token != NULL) {
				if (isdigit(token[0])) {
	...
	file = fopen(WfbLogFile,
		"w"); // Open the file in write mode, truncating it to zero size
	if (file != NULL)
		fclose(file);

	if (false /*out_sock>0*/) { // if we have outbound socket for mavlink, but
								// now if available it is used for MSP
		sprintf(msg_buf, "%d video pckts dropped!\n", total_dropped_packets);
		printf("%s", msg_buf);

		mavlink_message_t message;

		mavlink_msg_statustext_pack_chan(system_id, MAV_COMP_ID_SYSTEM_CONTROL, MAVLINK_COMM_1,
			&message,
			4, // 4 - Warning, 5 - Error
			msg_buf, 0, 0);
...
int GetTXTemp() {
	...
	if (glob("/proc/net/*/*/thermal_state", GLOB_NOSORT, NULL, &glob_result) == 0) {
		if (glob_result.gl_pathc > 0) {
			...
			if (stat == NULL) {
				fprintf(
					stderr, "Failed to open the thermal_state file: %s\n", glob_result.gl_pathv[0]);
			...
			if (fgets(buffer, sizeof(buffer), stat) != NULL) {
				if (sscanf(buffer,
						"rf_path: %*d, thermal_value: %*d, offset: %*d, "
						"temperature: %d",
						&temperature) == 1) {
...
void ProcessChannel(int rc_channel_no) {
	...
	uint16_t val = 0;
	if (rc_channel_no < 0 ||
		rc_channel_no > 15 /* || (mavpckts_ttl<100*/) // wait in the beginning for
													  // the values to settle
		return;
	...
	if (abs(val - NewValue[rc_channel_no]) > 32 && ChannelPersistPeriodMS > 0) {
		...
		return;
	} else if (abs(get_current_time_ms() - NewValueStart[rc_channel_no]) < ChannelPersistPeriodMS)
		return; // New value should remain "stable" for a second before being
				// approved
	else {
	} // New value persisted for more THAN ChannelPersistPeriodMS
	...
	// For compatibility purposes, channels.sh is called with 1-based channel
	// argument.
	...
	if (ChannelCmds[rc_channel_no] > 0) {
		// intentionally skip the first command, since when starting  it will
		// always receive some channel value and execute the script
		system(buff);
	}
	...
	for (int i = 0; i < count; ++i) {
		...
		if (mavlink_parse_char(MAVLINK_COMM_0, buffer[i], &message, &status) == 1) {
			...
			if (aggregate > 0) { // We will send whole packets only
				if (((aggregate >= 1 && aggregate < 50) &&
						mavpckts_count >= aggregate) || // if packets more than treshold
					((aggregate > 50 && aggregate < 2000) &&
						mavbuff_offset >= aggregate) || // if buffer  more than treshold
					((mavpckts_count >= minAggPckts) &&
						message.msgid == MAVLINK_MSG_ID_ATTITUDE) // MAVLINK_MSG_ID_ATTITUDE
...
static void serial_read_cb(struct bufferevent *bev, void *arg) {
	...
	while ((in_len = evbuffer_get_length(input))) {
		...
		if (ParseMSP) {
			...
			// sendto(out_sock, data, packet_len, 0, (struct sockaddr
			// *)&sin_out, sizeof(sin_out));
			...
			if (!version_shown &&
				ttl_packets % 10 == 3) // If garbage only, give some feedback do diagnose
...
static void *setup_temp_mem(off_t base, size_t size) {
	...
	// Enable the T-Sensor by configuring MISC_CTRL45 bit[31] and start to
...
static bool ReadSerialSimple(int showstat) {
	...
	// support for very low refresh rate, we will lose some FC data, may end up
	// with distorted OSD? But we need to read all data in the beginning to
	// identify the FC !
...
static int handle_data(const char *port_name, int baudrate, const char *out_addr) {
	...
	if (strlen(out_addr) > 1) {
		...
		if (!parse_host_port(
				out_addr, (struct in_addr *)&sin_out.sin_addr.s_addr, &sin_out.sin_port))
			goto err;
	}
	...
	struct event *stdin_event =
		event_new(base, STDIN_FILENO, EV_READ | EV_PERSIST, stdin_read_callback, base);
