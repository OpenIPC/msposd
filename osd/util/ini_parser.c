...
void save_value_to_system(const char *save_command, const char *value) {
    ...
    const char *placeholder_pos = strstr(save_command, placeholder);  // Find the position of the placeholder
    ...
    if (placeholder_pos) {
        ...
        // Append the value
        strncat(formatted_command, value, sizeof(formatted_command) - strlen(formatted_command) - 1);

        // Append the part after the placeholder
        strncat(formatted_command, placeholder_pos + strlen(placeholder), sizeof(formatted_command) - strlen(formatted_command) - 1);
    } else {
    ...
    while (fgets(line, sizeof(line), file)) {
       ...
       // Ignore empty lines
        ...
        // Check for Options
            ...
            // Check if the value is a list or a range and initialize current value index
            ...
            if (strlen(option->read_command) > 0) {
                ...
                // Process the output to match the current value index in the list or range
                    ...
                    case MENU_OPTION_LIST: {
                        ...
                        for (int i = 0; i < value_count; i++) {
                            if (strcmp(output, value_list[i]) == 0) {
                                current_section->current_value_index[current_section->option_count - 1 ] = i;
                                break;
                            }
                        }
                        break;
