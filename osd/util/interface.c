...
void parse_line(char *line, FrequencyChannel *fc, int *count) {
	...
	// Try to match the line to the format: * <frequency> MHz [<channel>] (dBm
	// info)
...
int query_interface_for_available_frequencies() {
	...
	while (fgets(buffer, sizeof(buffer), fp) != NULL) {
		...
		// If we're in the Frequencies section, parse the lines with
	...
	if (count > 0) {
		...
		for (int band_index = 0; band_index < FREQ_LABEL_SIZE; band_index += BAND_NAME_LENGTH) {
			...
			channelFreqLabel[band_index + 5] =
				'1' + (band_index / BAND_NAME_LENGTH); // This appends the number '1' through '8'
