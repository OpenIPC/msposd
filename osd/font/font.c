...
void get_font_path_with_extension(char *font_path_dest, const char *font_path,
	const char *extension, uint8_t len, uint8_t is_hd, const char *font_variant) {
	char name_buf[len];
	char res_buf[len];
...
static int open_font(const char *filename, display_info_t *display_info, const char *font_variant) {
	...
	if (ihdr.height != display_info->font_height * NUM_CHARS) {
		printf("font invalid height, got %d wanted %d\n", ihdr.height,
			display_info->font_height * NUM_CHARS);
		goto err;
	}
	...
	for (int page = 0; page < num_pages; page++) {
		DEBUG_PRINT(
			"Loading font page %d of %d, placing %x\n", page, num_pages, display_info->fonts);
		display_info->fonts[page] = malloc(
			display_info->font_width * display_info->font_height * NUM_CHARS * BYTES_PER_PIXEL);
		DEBUG_PRINT("Allocated %d bytes for font page buf at%x\n",
			display_info->font_width * display_info->font_height * NUM_CHARS * BYTES_PER_PIXEL,
			display_info->fonts[page]);
		for (int char_num = 0; char_num < NUM_CHARS; char_num++) {
			for (int y = 0; y < display_info->font_height; y++) {
				// Copy each character line at a time into the correct font
				// buffer
				int char_width_bytes = display_info->font_width * BYTES_PER_PIXEL;
				int char_size_bytes_dest =
					(display_info->font_width * display_info->font_height * BYTES_PER_PIXEL);
				int char_size_bytes_src =
					(ihdr.width * display_info->font_height * BYTES_PER_PIXEL);
				memcpy((uint8_t *)display_info->fonts[page] + (char_num * char_size_bytes_dest) +
						   (y * char_width_bytes),
					(uint8_t *)font_data + (char_num * char_size_bytes_src) +
						(ihdr.width * y * BYTES_PER_PIXEL) + (page * char_width_bytes),
					char_width_bytes);
			}
		}
	}
...
void load_font(display_info_t *display_info, const char *font_variant) {
	...
	if (display_info->fonts[0] == NULL) {
		...
		// finally, if we have no fonts for this FC, fallback to the default
		// font
		if (loaded_font) {
