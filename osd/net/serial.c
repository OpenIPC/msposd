...
int open_pty(const char **pty_name) {
	...
	// openpty makes a connection between the first argument, a parent FD which
	// acts as the "receiving" end of the virtual PTY and the second argument, a
	// child FD which represents a "virtual serial port." using ttyname on the
	// second argument will return a /dev which can be opened as a serial port
