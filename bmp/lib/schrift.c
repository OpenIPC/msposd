...
static int hor_metrics(
	SFT_Font *font, uint_fast32_t glyph, int *advanceWidth, int *leftSideBearing);
...
static int simple_flags(
	SFT_Font *font, uint_fast32_t *offset, uint_fast16_t numPts, uint8_t *flags);
static int simple_points(
	SFT_Font *font, uint_fast32_t offset, uint_fast16_t numPts, uint8_t *flags, Point *points);
static int decode_contour(
	uint8_t *flags, uint_fast16_t basePoint, uint_fast16_t count, Outline *outl);
static int simple_outline(
	SFT_Font *font, uint_fast32_t offset, unsigned int numContours, Outline *outl);
...
/* Loads a font from the file system. To do so, it has to map the entire font
 * into memory. */
	...
	while (numTables > 0) {
		...
		if (format == 0 && (flags & HORIZONTAL_KERNING) && !(flags & MINIMUM_KERNING)) {
			...
			if ((match = bsearch(key, sft->font->memory + offset, numPairs, 6, cmpu32)) != NULL) {
				...
				value =
					geti16(sft->font, (uint_fast32_t)((uint8_t *)match - sft->font->memory + 4));
...
static void transform_points(unsigned int numPts, Point *points, double trf[6]) {
	...
	for (i = 0; i < numPts; ++i) {
		pt = points[i];
		points[i] =
			(Point){pt.x * trf[0] + pt.y * trf[2] + trf[4], pt.x * trf[1] + pt.y * trf[3] + trf[5]};
	}
}
...
static int grow_points(Outline *outl) {
	...
	/* Since we use uint_fast16_t for capacities, we have to be extra careful
	 * not to trigger integer overflow. */
...
/* Like bsearch(), but returns the next highest element if key could not be
 * found. */
static void *csearch(const void *key, const void *base, size_t nmemb, size_t size,
	int (*compar)(const void *, const void *)) {
...
static int gettable(SFT_Font *font, char tag[4], uint_fast32_t *offset) {
	...
	/* No need to bounds-check access to the first 12 bytes - this gets already
	 * checked by init_font(). */
...
static int cmap_fmt4(SFT_Font *font, uint_fast32_t table, SFT_UChar charCode, SFT_Glyph *glyph) {
	...
	/* Look up segment info from the arrays & short circuit if the spec
	 * requires.
	 */
	...
	idOffset =
		idRangeOffsets + segIdxX2 + idRangeOffset + 2U * (unsigned int)(shortCode - startCode);
...
static int cmap_fmt12_13(
	SFT_Font *font, uint_fast32_t table, SFT_UChar charCode, SFT_Glyph *glyph, int which) {
...
/* For a 'simple' outline, determines each point of the outline with a set of
 * flags. */
static int simple_flags(
	SFT_Font *font, uint_fast32_t *offset, uint_fast16_t numPts, uint8_t *flags) {
...
/* For a 'simple' outline, decodes both X and Y coordinates for each point of
 * the outline. */
static int simple_points(
	SFT_Font *font, uint_fast32_t offset, uint_fast16_t numPts, uint8_t *flags, Point *points) {
...
static int decode_contour(
	...
	/* Skip contours with less than two points, since the following algorithm
	 * can't handle them and they should appear invisible either way (because
	 * they don't have any area). */
	if (count < 2)
		return 0;
	...
	if (flags[0] & POINT_IS_ON_CURVE) {
		...
		outl->points[outl->numPoints++] =
			midpoint(outl->points[basePoint], outl->points[basePoint + count - 1]);
	}
	beg = looseEnd;
	gotCtrl = 0;
	for (i = 0; i < count; ++i) {
		/* cur can't overflow because we ensure that basePoint + count < 0xFFFF
		 * before calling decode_contour(). */
		cur = (uint_least16_t)(basePoint + i);
		/* NOTE clang-analyzer will often flag this and another piece of code
		 * because it thinks that flags and outl->points + basePoint don't
		 * always get properly initialized -- even when you explicitly loop over
		 * both and set every element to zero (but not when you use memset).
		 * This is a known clang-analyzer bug:
...
static int simple_outline(
	 ...
	 * Falling endPts have no sensible interpretation and most likely only occur
	 * in malicious input. Therefore, we bail, should we ever encounter such
	 * input. */
...
static int compound_outline(SFT_Font *font, uint_fast32_t offset, int recDepth, Outline *outl) {
	...
	/* Guard against infinite recursion (compound glyphs that have themselves as
	 * component). */
	if (recDepth >= 4)
		return -1;
	do {
		...
		offset += 4;
		/* We don't implement point matching, and neither does stb_truetype for
		 * that matter. */
		...
		/* At this point, Apple's spec more or less tells you to scale the
		 * matrix by its own L1 norm. But stb_truetype scales by the L2 norm.
		 * And FreeType2 doesn't scale at all. Furthermore, Microsoft's spec
		 * doesn't even mention anything like this. It's almost as if nobody
		 * ever uses this feature anyway. */
...
/* A heuristic to tell whether a given curve can be approximated closely enough
 * by a line. */
static int is_flat(Outline *outl, Curve curve) {
